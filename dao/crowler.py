...
class Crowler:
    ...
    class NATEPANN(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('https://pann.nate.com/talk/c20030', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup

        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.a.text.strip() for title in soup.select('.subject')]
            links = ['https://pann.nate.com' + link.a['href'] for link in soup.select('.subject')]
            return titles, links
    ...
    class DCINSIDE(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('https://gall.dcinside.com/board/lists?id=issuezoom', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup

        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.a.text.strip() for title in soup.select('.gall_tit')][1:]
            links = ['https://gall.dcinside.com' + link.a['href'] for link in soup.select('.gall_tit')][1:]
            return titles, links
    ...
    class RULIWEB(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('https://bbs.ruliweb.com/best/humor/hit?orderby=readcount&range=24h', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup

        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.a.text.strip() for title in soup.select('.subject')[1:]]
    ...
    class PPOMPPU(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('http://www.ppomppu.co.kr/zboard/zboard.php?id=humor', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup

        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.text.strip() for title in soup.select('.list_title')[1:]]
            links = ['http://www.ppomppu.co.kr/zboard/' + link.parent['href'] for link in soup.select('.list_title')[1:]]
            return titles, links
    ...
    class FMKOREA(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('https://www.fmkorea.com/index.php?mid=humor&sort_index=pop&order_type=desc&listStyle=list&page=1', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup

        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.a.text.strip() for title in soup.select('.title.hotdeal_var8')]
            links = ["https://www.fmkorea.com/" + link.a['href'] for link in soup.select('.title.hotdeal_var8')]
            return titles, links
    ...
    class TODAYHUMOR(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('http://www.todayhumor.co.kr/board/list.php?table=bestofbest&page=1', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup
        ...
        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.a.text.strip() for title in soup.select('.subject')]
            links = ["http://www.todayhumor.co.kr" + link.a['href'] for link in soup.select('.subject')]
            return titles, links
    ...
    class YGOSU(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('https://www.ygosu.com/community/real_article', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup
    ...
    class HUMORUNIV(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('http://web.humoruniv.com/board/humor/list.html?table=pick', headers = header)
        ...
        def get_board(self):
            ...
            links = ['http://web.humoruniv.com/board/humor/' + link.a['href'] for link in soup.select('.li_sbj')]
            return titles, links
    ...
    class ETOLAND(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('http://www.etoland.co.kr/bbs/board.php?bo_table=etohumor03&sfl=top_n&stx=day&sst=wr_hit&sod=desc', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup

        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.a.next_sibling.next_sibling.next_sibling.next_sibling.span.text for title in soup.select('.mw_basic_list_subject')]
            links = ["http://www.etoland.co.kr/" + link.a.next_sibling.next_sibling.next_sibling.next_sibling['href'][2:] for link in soup.select('.mw_basic_list_subject')]
            return titles, links
    ...
    class INVEN(Site):
        ...
        def get_beautifulsoup(self, link=None):
            ...
            if link:
                ...
                html_text = requests.get('http://www.inven.co.kr/board/webzine/2097?my=chu&category=%EC%9C%A0%EB%A8%B8&sort=PID&iskin=webzine', headers = header).text
            soup = BeautifulSoup(html_text, features='html.parser')
            return soup

        def get_board(self):
            soup = self.get_beautifulsoup()
            titles = [title.text.strip()[5:] for title in soup.select('.sj_ln')]
    ...
    site_list = sorted(['NATEPANN', 'DCINSIDE', 'RULIWEB', 'PPOMPPU', 'FMKOREA', 'TODAYHUMOR', 'YGOSU', 'HUMORUNIV', 'ETOLAND', 'INVEN'])
